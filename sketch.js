...
function setup() {
  createCanvas(2*m.length*lenScalar+50, 2*m.length*lenScalar+50);
...
function draw() {
  ...
  colorMode(RGB);
  if (frameCount > 1) {
    buffer.stroke(	255, 118, 118, 10);
    buffer.line(lenScalar*prev1.x, lenScalar*prev1.y, lenScalar*pos[pos.length-3].x, lenScalar*pos[pos.length-3].y);
    buffer.stroke(118, 255, 118, 50);
    buffer.line(lenScalar*prev2.x, lenScalar*prev2.y, lenScalar*pos[pos.length-2].x, lenScalar*pos[pos.length-2].y);
    buffer.stroke(144, 202, 249, 100);
    buffer.line(lenScalar*prev3.x, lenScalar*prev3.y, lenScalar*pos[pos.length-1].x, lenScalar*pos[pos.length-1].y);
    }
  prev1 = pos[pos.length-3].copy();
  prev2 = pos[pos.length-2].copy();
  prev3 = pos[pos.length-1].copy();  
}
